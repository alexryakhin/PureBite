...
public final class KTextStyle {
    ...
    public var swiftUIFont: Font {
        guard let font else {
            return Font
                .system(size: size)
                .weight(weight?.swiftUIWeight ?? .regular)
        }
        return Font(font)
    }
#endif
...
extension Array where Element == KTextStyle {

    func textStyle(named name: String?) -> KTextStyle? {
...
public extension Text {
    @inlinable func textStyle(_ style: KTextStyle?) -> Text {
        return self
            .font(style?.swiftUIFont)
            .kerning(style?.letterSpacing ?? 0)
    }
}
