...
//swiftlint:disable identifier_name, force_cast todo file_length force_unwrapping
...
open class DictionaryDecoder {
    ...
    public enum KeyDecodingStrategy {
        ...
        /// This means the result is consistent regardless of the current user's locale and language preferences.
        ///
        ...
        /// Provide a custom conversion from the key in the encoded Dictionary to the keys
        /// specified by the decoded types.
        /// The full path to the current decoding position is provided for context
        ...
        /// If the result of the conversion is a duplicate key,
        /// then only one value will be present in the container for the type to decode from.
...
private struct DictionaryCodingKeyedDecodingContainer<K: CodingKey> : KeyedDecodingContainerProtocol {
    ...
    fileprivate init(referencing decoder: _DictionaryDecoder, wrapping container: [String: Any]) {
        ...
        case .convertFromSnakeCase:
            ...
            // If we hit a duplicate key after conversion,
            // then we'll use the first one we saw. Effectively an undefined behavior with Dictionary dictionaries.
            self.container = Dictionary(container.map { key, value in
                (DictionaryDecoder.KeyDecodingStrategy._convertFromSnakeCase(key), value)
            }, uniquingKeysWith: { first, _ in first })
        case .custom(let converter):
            self.container = Dictionary(container.map { key, value in
                (converter(decoder.codingPath + [DictionaryCodingKey(stringValue: key, intValue: nil)]).stringValue,
                 value)
            }, uniquingKeysWith: { first, _ in first })
        }
    ...
    public func nestedContainer<NestedKey>(keyedBy type: NestedKey.Type, forKey key: Key) throws
            ...
            throw DecodingError.keyNotFound(
                ...
                DecodingError.Context(codingPath: self.codingPath,
                                      debugDescription: """
                                        ...
                                        no value found for key \(_errorDescription(of: key))
                                        """))
        }
        ...
        let container = DictionaryCodingKeyedDecodingContainer<NestedKey>(referencing: self.decoder,
                                                                          wrapping: dictionary)
        return KeyedDecodingContainer(container)
    }
    ...
    public func nestedUnkeyedContainer(forKey key: Key) throws -> UnkeyedDecodingContainer {
        ...
        guard let value = self.container[key.stringValue] else {
            throw DecodingError.keyNotFound(
                ...
                DecodingError.Context(codingPath: self.codingPath,
                                      debugDescription: """
                                        Cannot get UnkeyedDecodingContainer -- no value found for
                                        key \(_errorDescription(of: key))
                                        """))
        }
        ...
        guard !self.isAtEnd else {
            throw DecodingError.valueNotFound(
                ...
                DecodingError.Context(
                    codingPath: self.decoder.codingPath + [DictionaryCodingKey(index: self.currentIndex)],
    ...
    public mutating func nestedContainer<NestedKey>(keyedBy type: NestedKey.Type) throws
        ...
        guard !self.isAtEnd else {
            throw DecodingError.valueNotFound(KeyedDecodingContainer<NestedKey>.self,
                                              DecodingError.Context(codingPath: self.codingPath,
                                                                    debugDescription: """
                                                                        Cannot get nested keyed
                                                                        container -- unkeyed container is at end.
                                                                        """))
        }
    ...
    public mutating func nestedUnkeyedContainer() throws -> UnkeyedDecodingContainer {
        ...
        guard !self.isAtEnd else {
            throw DecodingError.valueNotFound(UnkeyedDecodingContainer.self,
                                              DecodingError.Context(codingPath: self.codingPath,
                                                                    debugDescription: """
                                                                        Cannot get nested keyed container --
...
extension _DictionaryDecoder {
    ...
    func unbox(_ value: Any, as type: Bool.Type) throws -> Bool? {
        ...
        if let number: NSNumber = value as? NSNumber {
            ...
            if number === kCFBooleanTrue as NSNumber {
                ...
                // TODO: Add a flag to disallow non-boolean numbers?
                return number != 0
            }

            /* FIXME: If swift-corelibs-foundation doesn't change to use NSNumber,
             this code path will need to be included and tested:
    ...
    fileprivate func unbox(_ value: Any, as type: Float.Type) throws -> Float? {
        ...
        if let number = value as? NSNumber, number !== kCFBooleanTrue, number !== kCFBooleanFalse {
            ...
            //   * and the integral value was <= Float.greatestFiniteMagnitude,
            //     we are willing to lose precision past 2^24
            ...
            /* FIXME: If swift-corelibs-foundation doesn't change to use NSNumber,
             ...
             if let float: Float = Float(exactly: int) {
             return float
             }
             overflow = true
             */
            ...
            case let .convertFromString(posInfString,
                                        negInfString,
    ...
    fileprivate func unbox(_ value: Any, as type: Double.Type) throws -> Double? {
        ...
        if let number = value as? NSNumber, number !== kCFBooleanTrue, number !== kCFBooleanFalse {
            ...
            /* FIXME: If swift-corelibs-foundation doesn't change to use NSNumber,
             ...
             if let double: Double = Double(exactly: int) {
             return double
             }
             overflow = true
             */
    ...
    fileprivate func unbox(_ value: Any, as type: UUID.Type) throws -> UUID? {
        ...
        // NB this could be dangerous - we're assuming that it's ok to call CFGetTypeID
        // with the value, which may not be true
