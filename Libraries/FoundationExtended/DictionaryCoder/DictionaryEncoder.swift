...
//swiftlint:disable force_unwrapping identifier_name force_cast
...
open class DictionaryEncoder {
    ...
    public enum KeyEncodingStrategy {
        ...
        /// Provide a custom conversion to the key in the encoded
        /// Dictionary from the keys specified by the encoded types.
        /// The full path to the current encoding position is provided
        /// for context (in case you need to locate this key within the payload).
    ...
    /// value is encountered during encoding, and the encoding strategy is `.throw`.
    ...
    open func encode<T: Encodable>(_ value: T) throws -> NSDictionary {
        ...
        if topLevel is NSNull {
            throw EncodingError.invalidValue(
                value,
                EncodingError.Context(codingPath: [],
                                      debugDescription: "Top-level \(T.self) encoded as null Dictionary fragment."))
        } else if topLevel is NSNumber {
            throw EncodingError.invalidValue(
                value,
                EncodingError.Context(codingPath: [],
                                      debugDescription: "Top-level \(T.self) encoded as number Dictionary fragment."))
        } else if topLevel is NSString {
            throw EncodingError.invalidValue(
                value,
                EncodingError.Context(codingPath: [],
                                      debugDescription: "Top-level \(T.self) encoded as string Dictionary fragment."))
        }
    ...
    /// floating-point value is encountered during encoding, and the encoding strategy is `.throw`.
    /// - throws: An error if any value throws an error during encoding.
...
private class _DictionaryEncoder: Encoder {
    ...
    public func container<Key>(keyedBy: Key.Type) -> KeyedEncodingContainer<Key> {
        ...
        let container = DictionaryCodingKeyedEncodingContainer<Key>(referencing: self,
    ...
    public func unkeyedContainer() -> UnkeyedEncodingContainer {
        ...
        return _DictionaryUnkeyedEncodingContainer(referencing: self,
                                                   codingPath: self.codingPath,
                                                   wrapping: topContainer)
    }
...
private struct DictionaryCodingKeyedEncodingContainer<K: CodingKey>: KeyedEncodingContainerProtocol {
    ...
    public mutating func nestedUnkeyedContainer(forKey key: Key) -> UnkeyedEncodingContainer {
        ...
        self.codingPath.append(key)
        defer { self.codingPath.removeLast() }
        return _DictionaryUnkeyedEncodingContainer(referencing: self.encoder,
                                                   codingPath: self.codingPath,
                                                   wrapping: array)
    }
    ...
    public mutating func superEncoder() -> Encoder {
        return _DictionaryReferencingEncoder(referencing: self.encoder,
                                             ...
                                             convertedKey: _converted(DictionaryCodingKey.super),
...
extension _DictionaryEncoder {
    ...
    func box(_ date: Date) throws -> NSObject {
        ...
        case .deferredToDate:
            ...
            // Dates encode as single-value objects; this can't both throw and push a container,
            // so no need to catch the error.
    ...
    // Because the return type here is different from all of the "box" overloads (and is more general),
    // any "box" calls in here would call back into "box" recursively instead of calling the appropriate overload,
...
/// _DictionaryReferencingEncoder is a special subclass of _DictionaryEncoder which has its own storage,
