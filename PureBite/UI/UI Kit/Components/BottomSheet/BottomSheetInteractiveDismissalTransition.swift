...
extension BottomSheetInteractiveDismissalTransition {
    ...
    private func restoreBottomSheetHeight(with contentView: UIView) {
        let heightAnimator = heightAnimator ?? makeHeightAnimator(
            animating: contentView, to: initialSheetHeight + 1
        ) { [weak self] _ in
            self?.heightAnimator = nil
        }

        heightAnimator.fractionComplete = 1
...
extension BottomSheetInteractiveDismissalTransition: UIViewControllerAnimatedTransitioning {
    ...
    private func prepareOffsetAnimatorForDismissTransition(
        ...
        let offsetAnimator = makeOffsetAnimator(
            ...
            // If a bottom sheet has not been closed after being dragged, restore the height
            if position != .end {
                self?.restoreBottomSheetHeight(with: view)
            }
